...
import sidescroller.entity.bullet.Bullet;
...
public class SideScroller extends Application{
	 ...
	 */
	private String title = "The Mighty Knight";
	/**
	...
	public void init() throws Exception{
		...
		player = new Player(width/2, height/5, 20*SCALE, 20*SCALE, playerSprite);
		
		player.onEPress(() -> {
			Tuple currentPos = player.getCurrentPos();
			Bullet bullet = new Bullet(currentPos.x(), currentPos.y(), playerSprite.getLeft(), SCALE, new BulletSprite());
			board.movingShapes().add(bullet);
		});
	...
	private Region createOptionsBar(){
		...
		CheckMenuItem grid = createCheckMenuItem( "Grid", false, board.drawGridProperty());
